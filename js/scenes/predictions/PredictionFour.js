...
export default class PredictionTwo extends Phaser.Scene {
    ...
    create(data){
        ...
        fortuneButton.on('pointerdown', () => {
            this.daveFortune = this.add.image(500, 300, 'fortuneEmelio');
        ...
        submitButton.on('pointerdown', () => { 
            ...
            if(this.answerOne.answer === 'The Hermit')
                correct++;
            if(this.answerTwo.answer === 'The Star')
                correct++;
            if(this.answerThree.answer === 'The Sun')
                correct++;
            switch(correct - preCorrect){
                case 3:
                    resultsMessage = "Emelio's cards were just right.";
                    break;
                case 2:
                    resultsMessage = "Emelio's reading lacked one crucial detail.";
                    break;
                case 1:
                    resultsMessage = "Emelio only received one good card.";
                    break;
                case 0:
                    resultsMessage = "Emelio's reading was all wrong.";
                    break;
            }
            data.results.push(resultsMessage);
            this.scene.start("PromptFive", {correct: correct, results: data.results});  
        });
